...
package GUS::Community::Plugin::LoadBLATProteinAlignments; 
...
sub run {
   ...
   die "LoadBLATAlignments: query_file not defined" unless (-e $queryFile);
   die "LoadBLATAlignments: unrecognized query_table_id"  unless $queryTableName =~ /^(?:AASequence|ExternalAASequence|Assembly|AssemblySequence|ExternalNASequence|VirtualSequence|SplicedNASequence)$/i;
...
sub loadAlignment {
   ...
   }else{ ##the queries are proteins and we'll make up values to pass in.
     ( $qualityId,$has3p,$is3p,$has5p,$is5p,$qn,$tn,$pctId,$alignedBases,
         $maxQGap,$maxTGap,$numSpans, $minQs, $maxQe, $ts, $te, $strand, $end3, $end5
       ) = ( 1,1,1,1,1,$align->get('q_name'),$align->get('t_name'),1,$alignedBases,1,1,$align->get('num_blocks'),$align->get(''),$align->get(''),$align->get('t_start'),$align->get('t_end'),$align->get('strand'),1,1 );
