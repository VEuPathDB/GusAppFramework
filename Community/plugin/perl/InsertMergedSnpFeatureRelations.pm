...
my $argsDeclaration =
  [
   ...
   booleanArg({ name  => 'outputNestedMerges',
		descr => "output nested merges as they are found; to facilitate RESTART",
		constraintFunc => undef,
		reqd           => 0,
		isList         => 0 }),
...
Restart with skip flag.  Plugin will not evaluate nested merges prior to the skip.

Nested merges (ones requiring a second pass to create NAFeatureRelations) can be output as they are 
found using the --outputNestedMerges flag.  File can then be used as the inFile to load only nested merges
...
sub run {
  ...
  my $rowCount = 0;
  my $nestedMergeFile = undef;

  my $skip = $self->getArg('skip');
  my $fName = $self->getArg('inFile');

  if ($self->getArg('outputNestedMerges')) {
    $nestedMergeFile = FileHandle->new;
    $nestedMergeFile->open(">$fName-nested-merges.txt") || $self->error("Unable to create nested merge file: $fName-nested-merges.txt");
  }
  ...
  while (<$fh>) {
    unless (++$rowCount % 5000) {
      $self->log("SKIPPED $rowCount rows.") if ($rowCount <= $skip) ;
    }
  ...
  $fh->close();
  $nestedMergeFile->close() if ($nestedMergeFile);
