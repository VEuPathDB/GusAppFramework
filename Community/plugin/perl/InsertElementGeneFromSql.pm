##
## InsertElementGeneFromSql Plugin
## $Id: $
##

package GUS::Community::Plugin::InsertElementGeneFromSql;
  ...
  my $argumentDeclaration  =
    [
     fileArg({name => 'sqlFile',
	      descr => 'The full path of a tab-delimited file whose 1st column contains sqls returning (element_id, gene_id) pairs. One sql statement per line with variable DoTS.Gene.external_database_release_id',
...
sub getDocumentation {
  my $purposeBrief = 'Loads Entrez Gene annotation for an array into RAD.ElementGene using sql queries.';
  
  my $purpose = "This plugin runs a set of sql queries returning (element_id, gene_id) pairs and populates RAD.ElementGene.";

  my $tablesAffected = [['RAD::ElementGene', 'Enters a row for each pair returned by an sql query.']];
...
sub insertElementGene {
  ...
  while (my $line=<$fh>) {
	...
	$self->log("Working on row $lineNum2.");
      ...
      my $elementGene = GUS::Model::RAD::ElementGene->new({element_id => $ceId, gene_id => $geneId});
      if (!$elementGene->retrieveFromDB()) {
	$elementGene->submit();
	$insertCount++;
      }
