...
# Nomalization for cDNA microarray data: a robust composite method addressing
...
use GUS::Model::RAD3::LogicalGroupLink;
use GUS::Model::RAD3::GenePixElementResult;
...
PURPOSE
  ...
  my $tablesAffected = 
    [['RAD3::LogicalGroup', 'Enters a row which will be used as analysis input for this normalization process'],
     ['RAD3::LogicalGroupLink', 'Enters 2 rows which correspond to the 2 input quantifications']];
  
    my $tablesDependedOn = 
      [['RAD3::Protocol', 'The analysis protocol used'],
       ['RAD3::ProtocolParam', 'The parameters for the protocol or its components'],
       ['Core::TableInfo', 'The table from which the table_id for RAD3.Quantification is retrieved'], 
       ['Core::DatabaseInfo', 'The table from which the database_id for RAD3 is retrieved']]; 
...
B<I<element_result_table>>
    The specific view of RAD3.ElementResultImp to which the element result data after quantification are loaded. FOR NOW THIS IS LIMITED TO GenePixElementResult.

The following parameters are optional:

B<I<flag_nameN>>
    The name of the I<N>th flag in the specified element_result_table which will be used to exclude elements whose flag value equals I<flag_valueN>. FOR NOW THIS IS LIMITED TO flag (from RAD3.GenePixElementResult)
...
NOTES
  ...
  my $argsDeclaration  =
    [
     ...
     stringArg({name => 'fg',
		descr => 'The name of the field to be used as foreground measurement; should be one of the foreground fields of the view of RAD3.ElementResultImp into which the quantifications were loaded.',
		constraintFunc => undef,
		reqd  => 1,
		isList => 0
	       }),
     stringArg({name => 'bg',
		descr => 'The name of the field to be used as background measurement. If specified, background subtraction will be performed. In this case, this argument should be one of the background fields of the view of RAD3.ElementResultImp into which the quantifications were loaded.',
		constraintFunc => undef,
		reqd  => 0,
		isList => 0
	       }),
  ...
  $self->initialize({requiredDbVersion => {Core => '3', RAD3 => '3'},
		     cvsRevision => '$Revision: 1622 $', # cvs fills this in!
...
sub run{
  ...
  $self->setResultDescr("Finished loess normalization. inserted 1 row in RAD3.logicalGroup and 2 rows in RAD3.logicalGroupLink");
...
#--------------
  ...
  $self->checkFg($self->getArgs()->{fg});
  if ($self->getArgs()->{bg}) {
    $self->checkBg($self->getArgs()->{bg});
  }
}
...
#--------------
  ...
  $cfg_rv->{fg} = $self->getArgs()->{fg};
  if ($self->getArgs->{bg}) {
    $cfg_rv->{bg} = $self->getArgs()->{bg};
  }
...
sub readArrayInfo{
  ...
  foreach my $ch_id (1..2) {
    my $k = $ch_id-1;
    $cfg_rv->{quantification_id}->[$k] = $self->getArgs()->{"qid$ch_id"};
    my $qt = GUS::Model::RAD3::Quantification->new({ quantification_id => $cfg_rv->{quantification_id}->[$k] });
    ...
    my $acq = GUS::Model::RAD3::Acquisition->new({ acquisition_id => $acq_id });
    
    if (!$acq->retrieveFromDB){
      $self->error("acquisition_id $acq_id does not exist in RAD3.");
    } 
    ...
    my $channel = GUS::Model::RAD3::Channel->new({channel_id => $channel_id});
  ...
  ## retrieve array_id, in order to get array layout information
  my $assay = GUS::Model::RAD3::Assay->new({ assay_id => $assay_id[1] });
  if (!$assay->retrieveFromDB){
    $self->error("Failed to retrieve from RAD3::Assay for assay_id = $assay_id[1]");
  }

  $cfg_rv->{array_id} = $assay->getArrayId;

  my $array = GUS::Model::RAD3::Array->new({ array_id => $cfg_rv->{array_id} });		
  if (!$array->retrieveFromDB){
    $self->error("Failed to retrieve from RAD3::Array for array_id = $cfg_rv->{array_id}");
  }
...
sub retrieveElementResult{
  ...
  my $sql;
  if ($cfg_rv->{bg}) {
    $sql = "SELECT element_id, $cfg_rv->{fg}, $cfg_rv->{bg} FROM RAD3.$cfg_rv->{element_result_table} WHERE quantification_id = ?";
  } else {
    $sql = "SELECT element_id, $cfg_rv->{fg} FROM RAD3.$cfg_rv->{element_result_table} WHERE quantification_id = ?";
  }
...
sub runLoessFromR{
  ...
  my $sql = "select element_id from rad3.spot where array_id = ? order by to_number(array_row), to_number(array_column), 
             to_number(grid_row), to_number(grid_column), to_number(sub_row), to_number(sub_column)";
    
  my $sth = $dbh->prepare($sql);
  $sth->execute($cfg_rv->{array_id});
  ...
  my $sm_param = $self->getArgs()->{smoothing_param};
...
sub createLogicalGroup{
  ...
  # retrieve study name
  my $sql = "SELECT s.name, a.name FROM RAD3.study s, rad3.studyassay sa, rad3.acquisition acq, rad3.quantification q, rad3.assay a
  ...
  my $logicalGroup = GUS::Model::RAD3::LogicalGroup->new
    ( { name => "$name",
	category => 'quantification' } 
    );
  $sth = $dbh->prepare("select table_id from Core.TableInfo t, Core.DatabaseInfo d where t.database_id=d.database_id and d.name='RAD3' and t.name='Quantification'");
  $sth->execute();
  my ($table_id) = $sth->fetchrow_array;

  foreach my $k (0..1) {
    my $logicalGroupLink = GUS::Model::RAD3::LogicalGroupLink->new
      ( { table_id => $table_id,
  ...
  $self->logData('RESULT', "1 row inserted into table RAD3.LogicalGroup: name='$name'");
  $self->logData('RESULT',"2 rows inserted into RAD3.LogicalGroupLink");
...
sub writeAnalysisCfgFile {
  ...
  my $normProtocol = GUS::Model::RAD3::Protocol->new( {name => $norm_protocol_name});

  unless ($normProtocol->retrieveFromDB) {
    $self->error("failed to retrieve from rad3.Protocol for name = '$norm_protocol_name'");
  }
  
  my $norm_protocol_id = $normProtocol->getProtocolId();

  my $flagProtocol = GUS::Model::RAD3::Protocol->new( {name => 'Filter spots using GenePix flags'});

  unless ($flagProtocol->retrieveFromDB) {
    $self->error("failed to retrieve from rad3.Protocol for name = 'Filter spots using GenePix flags'");
  }
  ...
  if ($num_flags) {
    ...
    my $seriesProtocol = GUS::Model::RAD3::Protocol->new( {name => $series_protocol_name});

    unless ($seriesProtocol->retrieveFromDB) {
      $self->error("failed to retrieve from rad3.Protocol for name = '$series_protocol_name'");
    }
  ...
  if ($self->getArgs()->{smoothing_parameter}) {
    $param_id =  $self->getProtocolParamId($norm_protocol_id, 'smoothing_param');
    push @params, [$param_id, $self->getArgs()->{smoothing_parameter}];
  }
  ...
  $self->logData('RESULT',"The config file $fname used for AnalysisResultLoader plug-in has been written");
}
...
#-----------------------
  ...
  unless ($protocolParam->retrieveFromDB) {
    $self->error("failed to retrieve from rad3.ProtocolParam for protocol_id=$protocol_id and name=$name");
  }
...
#-----------
  ...
  my $resultTable = GUS::Model::RAD3::GenePixElementResult->new();
