##
## InsertAffy2GeneFromTable Plugin
## $Id: $
##

package GUS::Community::Plugin::InsertAffy2GeneFromTable;
...
sub getArgumentsDeclaration {
  my $argumentDeclaration  =
    [
     fileArg({name => 'inFile',
	      descr => 'The full path of a tab delimited text files with two columns (no header): Affymetrix ID, Entrez Gene ID',
...
sub getDocumentation {
  ...
  my $purpose = "This plugin reads a file mapping Affymetrix IDs to Entrez Gene IDs for a specified array and populates RAD.CompositeElementGene for that array.";
  ...
  my $howToRestart = "Loading can be resumed using the I<--restart n> argument where n is the line number in the annotation file of the first row to load upon restarting. Alternatively, one can use the plugin GUS::Community::Plugin::Undo to delete all entries inserted by a specific call to this plugin. Then this plugin can be re-run from fresh.";
