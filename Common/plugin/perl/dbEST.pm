...
sub run {
  ...
  if ($M->getCla->{update_file}) {
	...
	last if $fh->eof();
        ...
        my ($eid) = split /\t/, $l;
	next unless ($eid);
        my $ests = $M->sql_get_as_hash_refs_lc("select * from dbest.est where id_est = $eid");
        foreach my $est ( @{$ests}) {
          $e->{$est->{id_est}}->{e} = $est;
        }
      }
    ...
    my $st = $dbh->prepareAndExecute("select max(id_est) from dbest.est");
...
sub processEntries {
  ...
  my $A = $M->sql_get_as_hash_refs_lc("select * from dbest.SEQUENCE where id_est in ($ids) order by id_est, local_id");
  if ($A) {
    foreach my $s (@{$A}){
  ...
  $ids = join "," , keys %$e;
  $A = $M->sql_get_as_hash_refs_lc("select * from dbest.CMNT where id_est in ($ids) order by id_est, local_id");
  if ($A) {
    foreach my $c (@{$A}){
...
sub getMostRecentEntry {
  ...
  my $re = $M->sql_get_as_hash_refs_lc("select * from dbest.EST where id_est = $e->{replaced_by}")->[0];
...
sub setTableCaches {
  ...
  $q = 'select name, taxon_id  from sres.TaxonName where taxon_id in (8,14,45,211,10730421,1124,87262,12996,2892,10730347)';
  $A = $M->sql_get_as_array_refs($q);
  foreach my $r (@$A) {
    $M->{taxon}->{$r->[0]} = $r->[1];
  }
...
sub newLibrary {
  my ($M,$e,$l) = @_;
  my $dbest_lib = $M->sql_get_as_hash_refs_lc("select * from dbest.library where id_lib = $e->{id_lib}")->[0];
