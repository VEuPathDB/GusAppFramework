...
package GUS::Supported::Plugin::InsertExternalDatabase;
...
Creates a new entry in table SRes.ExternalDatabase to represent a new source of data imported into GUS.
PURPOSEBRIEF
    
my $purpose = <<PLUGIN_PURPOSE;
Simple plugin that is the easiest way to create a row representing a new database that exists in the outside world that will be imported into GUS.  This entry serves as a stable identifier across multiple releases of the database (which are stored in SRes.ExternalDatabaseRelease and point back to the entry created by this plugin).  Protects against making multiple entries in GUS for an external database that already exists there (see notes below).
PLUGIN_PURPOSE
