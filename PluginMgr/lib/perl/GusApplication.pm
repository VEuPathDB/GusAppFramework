...
sub initialize {
	...
	$M->setRequiredDbVersion({ Core => '3' });
	$M->setVersion('0.2');
	$M->setDescription('GusApplication');
	$M->setName(ref $M); #?
	$M->setEasyCspOptions();
...
sub findAlgorithm {
  ...
  my $P = shift;
	...
	$P->setAlgorithm($alg_gus->getId ? $alg_gus : undef)
}
...
sub findImplementation {
  ...
  my $P = shift;
	...
	$P->setImplementation($implementation) if $implementation;
...
SQL
	...
	# none found
	if (scalar @$imps == 0) {
		$P->setStatus("No matching Core.AlgorithmImplementation found for exe=$e ver=$v");
	...
	# too many found
		...
		$P->setStatus("Found more than one Core.AlgorithmImplementation found for exe=$e ver=$v");
		$P->log('ERROR-IMP', $M->getStatus);
		foreach (@$imps) {
			$P->log('ERROR-IMP', $_->{ALGORITHM_IMPLEMENTATION_ID}, $_->{VERSION});
		}
		$P->setOk(0);
	}
	...
	# just right
		...
		if (!$RV->retrieveFromDB) {
			...
			CBIL::Util::CBIL::Util::Disp:: Display($imps->[0]);
		}
	}
...
sub doMajorMode_Meta {
	...
	my $cla = CBIL::Util::CBIL::Util::EasyCsp::DoItAll($ecd,$M->getUsage);
	if ($cla) {
		$M->setCla($cla);
	}
	else {
	...
	->new({ version        => $M->getVersion,
					executable     => $M->getFile,
...
sub doMajorMode_Run {
  ...
  unless ($pu) {
	...
	my $cla = CBIL::Util::CBIL::Util::EasyCsp::DoItAll($ecd,$pu->getUsage);
	if ($cla) {
		$pu->setCla($cla);
		$M->setCla($cla);
	}
	...
	if ($M->openInvocation($pu)) {
		...
		# run
		$pu->setStatus($pu->run({ cla      => $pu->getCla,
...
sub doMajorMode_History {
	...
	my $cla = CBIL::Util::CBIL::Util::EasyCsp::DoItAll(\%ecd,$usg) || exit 0;
	$M->setCla($cla);
...
sub create_or_update_implementation {
	...
	CBIL::Util::CBIL::Util::Disp:: Display(\%ecd) if FLAG_DEBUG;
	my $cla = CBIL::Util::CBIL::Util::EasyCsp::DoItAll(\%ecd,$usg) || exit 0;
	$M->setCla($cla);
	...
	my $version = $pu->getVersion;
	if (not defined $version) {
		print STDERR "You have not called the setVersion method in your plugin's initialize method.\n";
		exit 0;
	}
	...
	CBIL::Util::CBIL::Util::Disp:: Display($apkt_cache, '$apkt_cache') if FLAG_DEBUG;
	my $pu_ecd     = {
										%{$pu->getGlobalEasyCspOptions},
										%{$pu->getEasyCspOptions},
									 };

	CBIL::Util::CBIL::Util::Disp:: Display($pu_ecd, '$pu_ecd') if FLAG_DEBUG;
	my @bad_types;
	...
	my $alg_imp_h = { version             => $version,
										description         => $imp_description,
										executable          => $host.':'.$pu->getFile,
	...
	if ($cla->{Survey}) {
		CBIL::Util::CBIL::Util::Disp:: Display($alg_h,     '$alg_h: ');
		CBIL::Util::CBIL::Util::Disp:: Display($alg_imp_h, '$alg_imp_h: ');
		CBIL::Util::CBIL::Util::Disp:: Display(\@apks,     '\@apks: ');
	}
...
#! GA
sub load_standard_runtime_cache {
	my $M = shift;

	# load mappings;
	# ........................................

	# load the mapping from type string to type id.
	#$M->load_mapping_table('prj2id', 'Project',   'project_id',   'name' );
	#$M->load_mapping_table('usr2id', 'UserInfo',  'user_id',      'login');
	#$M->load_mapping_table('grp2id', 'GroupInfo', 'group_id',     'name' );
	#$M->load_mapping_table('tag2id', 'AlgorithmParamKeyType',
	#											 'algorithm_param_key_type_id', 'type' );
	#$M->load_mapping_table('alg2id', 'Algorithm',	'algorithm_id', 'name' );
	#$M->load_mapping_table('tbl2id', 'TableInfo',	'table_id',     'table_name' );

	# map somethings we already have.
	$M->{userId}    = $M->{usr2id}->{$M->{user}};
	$M->{groupId}   = $M->{grp2id}->{$M->{group}};
	$M->{projectId} = $M->{prj2id}->{$M->{project}};
...
sub set_defaults {
	...
	CBIL::Util::CBIL::Util::Disp:: Display($cla, 'cla:'.  ref$M) if FLAG_DEBUG;
...
sub openInvocation {
	...
	#CBIL::Util::CBIL::Util::Disp:: Display($cla, '$cla');
	#CBIL::Util::CBIL::Util::Disp:: Display([sort keys %key_to_obj], '[sort keys %key_to_obj');
...
#							 );
	...
	if (scalar @any_bad > 0) {
		$P->setStatus('Could not record parameter values for '. join(', ',@any_bad));
		$P->setOk(0);
	}
...
sub connect_to_database {
  ...
  my $P = shift; # active plugins
	...
	my $login    = $P->getConfig->lookup('', 'gus/login');
	my $password = $P->getConfig->lookup('', 'gus/password');
	my $database = $P->getCla->{database} ||
  $P->getConfig->lookup(undef, 'gus/database');
	my $server   = $P->getCla->{server}   ||
  $P->getConfig->lookup(undef, 'gus/server');

	# JC: eliminate trailing whitespace from DB name
	$database =~ s/\s+$//;
	$database = $database ? $database : 'GUSdev';

	$P->setDb(new GUS::ObjRelP::DbiDatabase(undef,
														$login,$password,
