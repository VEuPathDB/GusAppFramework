...
sub run {
    ...
    my $view = "GUS::Model::$self->getArgs->{'subclass_view'}";
    eval "require $view";
    ...
    $resultDescrip .= "\n".$self->insertAnalysisResults($view, $analysis_id, $cfg_info->{'table_id'}, $data, $line_count);
...
sub checkViewName {
    ...
    if ($name =~ /^RAD3::(\w+)$/) {
	my $table = GUS::Model::Core::TableInfo->new({name => $1, is_view => 1, view_on_table_id => $id});
	if (!$table->retrieveFromDB) {
	    return "$name is not a valid view of RAD3.AnalysisResultImp.";
	}
    }
    else {
...
sub readCfgFile {
    ...
    my $fh = new IO::File;
    unless ($fh->open("<$self->getArgs->{'cfg_file'}")) {
	$self->error("Could not open file $self->getArgs->{'cfg_file'}.");
    }
    my $table_given = 0;
    ...
    if (scalar(@{$cfg_info->{'protocol_param_id'}}) != scalar(@{$cfg_info->{'protocol_param_value'}})) {
    ...
    if (scalar(@{$cfg_info->{'protocol_qc_param_id'}}) != scalar(@{$cfg_info->{'protocol_qc_param_value'}})) {
...
sub readDataFile {
    ...
    my $fh = new IO::File;
    unless ($fh->open("<$self->getArgs->{'data_file'}")) {
...
sub insertAnalysis {
    ...
    if (scalar(@{$cfg_info->{'protocol_param_id'}})) {
	for (my $i=1; $i<=@{$cfg_info->{'protocol_param_id'}}; $i++) {
	    if (defined $cfg_info->{'protocol_param_id'}->[$i]) {
		if (!defined $cfg_info->{'protocol_param_value'}->[$i]) {
		    $self->userError("Missing value for protocol_param $i in the cfg_file.");
		}
		else {
    ...
    if (scalar(@{$cfg_info->{'protocol_qc_param_id'}})) {
	for (my $i=1; $i<=@{$cfg_info->{'protocol_qc_param_id'}}; $i++) {
	    if (defined $cfg_info->{'protocol_qc_param_id'}->[$i]) {
		if (!defined $cfg_info->{'protocol_qc_param_value'}->[$i]) {
		    $self->userError("Missing value for protocol_qc_param $i in the cfg_file.");
		}
		else {
    ...
    $analysis->submit();
    $resultDescrip .= "Inserted $num_analysis_input rows in RAD3.AnalysisInput, $num_analysis_param rows in RAD3.AnalysisParam, $num_analysis_qc_param rows in RAD3.AnalysisQCParam.";
...
sub insertAnalysisResults {
    ...
    my $resultDescrip;
    my $num_results = 0;
    $self->getArgs->{'subclass_view'} =~ /^RAD3::(\w+)$/;
    my $subclass_view = $1;
